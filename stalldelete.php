...
mysqli_query($con,"DELETE FROM stall WHERE stall_id=$stall_id");

//if unable to execute the query, then give alert message
if(mysqli_affected_rows($con)<=0)
{
    echo "<script>alert('Unable to delete data!');";
    die("window.location.href='managerview.php';</script>");
}
...
echo "<script>window.location.href='managerview.php';</script>";
?>
