...
<?php
    ...
    $uid = $_GET['stall_id'];
    //write the sql query
    $sql = "Select * from stall where stall_id = $uid";
...
<form method="post" action="stallupdate.php">
  <table style="text-align:center">
    <tr><th width="300px">ID</th>
      <td><input type="text" name="uid" value="<?php echo $rows['stall_id']  ?>" readonly/></td>
    </tr>
<tr><th>Name:</th><td width="300px">
  <input type="text" name="name1" value="<?php echo $rows['stallname']?>"required="required"/>
</td>
</tr>
<tr><th>Email Address</th><td>
  <input name="email1" required="required" value="<?php echo $rows['stallemail']?>"/>
</td>
</tr>
...
<center>
  ...
  <input type="submit"value="Back to Previous Page" class="btn" formaction="stallview.php"/>
</center>
</td>
</tr>
</table>
...
<?php
  ...
  window.location.href='stallview.php'</script>");
}
?>
<!--------------------------------------- seperate parts------------------------------------------------->
